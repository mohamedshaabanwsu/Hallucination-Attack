    ...
    def run(self):
        ...
        while self.temp_step <= self.steps:
            ...
            '''if self.temp_output != self.target:
                ...
                self.mind_used = True
                print("Perturbed String got Target String, Attempting to get MIND target now WITH mind and llm loss\n")
                self.slice()
                self.grad()
                self.sample()
                self.forward_both()
            ...
            if not self.has_reached_target:
                if self.temp_output == self.target:
                    ...
                    self.mind_used = True
                    self.test_both_llm_mind()
                    print("Target string reached for the first time; from now on using MIND + LLM loss")
                    
            if not self.has_reached_target:
                # Steps using JUST LLM loss (trying to get initial target string)
                print("Trying to get to Target String\n")
                self.mind_used = False
                self.slice()
                self.grad()
                ...
                self.mind_used = True
                print("Using both LLM and MIND loss from now on\n")
                self.slice()
                self.grad()
                self.sample()
                self.forward_both()
